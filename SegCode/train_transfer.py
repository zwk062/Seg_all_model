...
args = {
    ...
    'snapshot': 100,
    'test_step': 1,
    'ckpt_path': 'D:/zhangchaoran/NEW_DATA_TRAIN/CT with MRI/to MRI/',
    'batch_size': 2,
}
...
opts=dict(title='to MRI loss', legend=['train loss', 'test loss'], xlabel='epochs', ylabel='loss_value'),env='final_ex')
    value_window = viz.line([[0,0,0,0]], [0],
opts=dict(title='to MRI value ', legend=['sen', 'spe','dsc','pre'], xlabel='epochs',ylabel='evaluation_value'),env='final_ex')
    # 再训练
...
# opts=dict(title='one layer retrain value', legend=['sen', 'spe', 'dsc', 'pre'], xlabel='epochs', ylabel='evaluation_value'))
    ...
    #迁移学习
    #加载预训练模型
    net = torch.load('D:/zhangchaoran/NEW_DATA_TRAIN/CT with MRI/our/vnet_Dice900.pkl')
    # 定义一个列表，其中包含希望训练的层的关键字
    train_layer_keywords = ["encoder1", "encoder2","down","final","dec1"]
    # 初始化变量来统计可训练参数的数量
    trainable_params = 0

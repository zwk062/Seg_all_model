...
class Reduction_Decoder(nn.Module):
    def __init__(self,input,d0,d1,d2,d3,d4,**kwargs):
        super(Reduction_Decoder, self).__init__(**kwargs)
        self.p1 =nn.Sequential(
            nn.Conv3d(input,d0[0],kernel_size=(1,1,1)),
            nn.ReLU(),
            nn.Conv3d(d0[0],d0[1],kernel_size=(5,5,5),padding=(2,2,2)),
            nn.ReLU()
        )
        ...
        self.p2 =nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(d1[0],d1[1],kernel_size=(1,1,7),padding=(0,0,3)),
            nn.ReLU()
        )
        self.p3 =nn.Sequential(
            nn.Conv3d(input, d2[0], kernel_size=(1, 1, 1)),
            nn.ReLU(),
            nn.Conv3d(d2[0], d2[1], kernel_size=(1, 7, 1),padding=(0,3,0)),
            nn.ReLU()
        ...
        self.p4 =nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(d3[0], d3[1], kernel_size=(7, 1, 1),padding=(3,0,0)),
            nn.ReLU()
        )
        self.p5 =nn.Sequential(
            nn.Conv3d(input, d4[0], kernel_size=(1, 1, 1),padding=(3, 3, 3)),
            nn.ReLU(),
    ...
    def forward(self,x):
        ...
        p5 = self.p5(x)
        # print(p5.shape)
        p6 = torch.cat((p2,p3,p4),dim=1)
        # print(p6.shape)
        return torch.cat((p1,p6,p5),dim=1)

class Muti_net(nn.Module):
    def __init__(self):
        super(Muti_net, self).__init__()
        self.encoder1 = Reduction_Encoder(1, (16,32), 16, (8,16,16,16))
        self.encoder2 = Reduction_Encoder(64,(32,64), 32, (16,32,32,32))
        self.encoder3 = Reduction_Encoder(128, (64,128), 64, (16,64,64,64))
        self.bridge = Reduction_Encoder(256, (128,256), 128, (64,128,128,128))
        ...
        self.decoder3 = Reduction_Decoder(512, (64,32),(128,64),(128,64),(128,64),(64,32,32,32))
        self.decoder2 = Reduction_Decoder(256, (32,16),(64,32),(64,32),(64,32),(32,16,16,16))
        self.decoder1 = Reduction_Decoder(128, (16,8),(32,16),(32,16),(32,16),(16,8,8,8,8))

...
def dense_block(x, nb_layers, growth_rate, isTraining, dropout_rate=None):
    ...
    for i in range(nb_layers):
        # Conv
        x = tf.layers.conv3d(inputs=concat_x, filters=growth_rate, kernel_size=3, strides=1, padding='same',
                              use_bias=False)
        # ReLU
        x = tf.nn.relu(x)
        # BN
...
def A_Dense_UNet(x, nb_classes, isTraining, dropout_rate, reuse=False):
    ...
    with tf1.variable_scope('A_Dense_UNet') as scope:
        ...
        with tf1.variable_scope('Dense_Block_1'):
            x = dense_block(x, nb_layers=6, growth_rate=12, isTraining=isTraining,
                            dropout_rate=dropout_rate)  # 16 + 72 = 88 (64*64*64)
        ...
        with tf1.variable_scope('Dense_Block_2'):
            x = dense_block(x, nb_layers=6, growth_rate=14, isTraining=isTraining,
                            dropout_rate=dropout_rate)  # 88 + 84 = 172 (32*32*32)
        ...
        with tf1.variable_scope('Dense_Block_3'):
            x = dense_block(x, nb_layers=6, growth_rate=16, isTraining=isTraining,
                            dropout_rate=dropout_rate)  # 172 + 96 = 268 (16*16*16)
        ...
        with tf1.variable_scope('Dense_Block_4'):
            x = dense_block(x, nb_layers=6, growth_rate=18, isTraining=isTraining,
                            dropout_rate=dropout_rate)  # 268 + 108 = 376 (8*8*8)
        with tf1.variable_scope('Cont_Block_1'):
            x = tf1.layers.conv3d_transpose(inputs=x, filters=384, kernel_size=2, strides=2, padding='same',
                                            use_bias=False)
            x = tf.nn.relu(x)
        ...
        with tf1.variable_scope('model_out_1'):
            out_1 = tf1.layers.conv3d_transpose(inputs=x, filters=276, kernel_size=4, strides=4, padding='same',
                                                use_bias=False)
            out_1 = tf.nn.relu(out_1)
            out_1 = tf1.layers.conv3d(inputs=out_1, filters=nb_classes, kernel_size=1, strides=1, padding='same',
                                      use_bias=False)

        with tf1.variable_scope('Atention_Block_2'):
            x = tf1.layers.conv3d_transpose(inputs=x, filters=276, kernel_size=2, strides=2, padding='same',
                                            use_bias=False)
            x = tf.nn.relu(x)
        ...
        with tf1.variable_scope('model_out_2'):
            out_2 = tf1.layers.conv3d_transpose(inputs=x, filters=180, kernel_size=2, strides=2, padding='same',
                                                use_bias=False)
            out_2 = tf.nn.relu(out_2)
            out_2 = tf1.layers.conv3d(inputs=out_2, filters=nb_classes, kernel_size=1, strides=1, padding='same',
                                      use_bias=False)

        with tf1.variable_scope('Attention_Block_3'):
            x = tf1.layers.conv3d_transpose(inputs=x, filters=180, kernel_size=2, strides=2, padding='same',
                                            use_bias=False)
            x = tf.nn.relu(x)
        ...
        with tf1.variable_scope('model_out_3'):
            out_3 = tf1.layers.conv3d(inputs=x, filters=nb_classes, kernel_size=1, strides=1, padding='same',
                                      use_bias=False)
...
def Dense_Net(x, y, isTraining, dropout_rate, reuse=False):
    ...
    with tf1.variable_scope('Dense_Net') as scope:
        ...
        with tf1.variable_scope('Input_layer') as scope:
            back_x = tf.reshape(x[:, :, :, :, 0],
                                shape=[x.get_shape()[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3], 1])
            fore_x = tf.reshape(x[:, :, :, :, 1],
                                shape=[x.get_shape()[0], x.get_shape()[1], x.get_shape()[2], x.get_shape()[3], 1])
            back_condition = tf.reshape(y[:, :, :, :, 0],
                                        shape=[y.get_shape()[0], y.get_shape()[1], y.get_shape()[2], y.get_shape()[3],
                                               1])
            fore_condition = tf.reshape(y[:, :, :, :, 1],
                                        shape=[y.get_shape()[0], y.get_shape()[1], y.get_shape()[2], y.get_shape()[3],
                                               1])
        ...
        with tf1.variable_scope('Transition_block_2') as scope:
            ...
            mid2 = tf1.layers.conv3d_transpose(inputs=x, filters=48, kernel_size=2, strides=2, padding='same',
                                               use_bias=False)
            out = mid1 + mid2
        return out
